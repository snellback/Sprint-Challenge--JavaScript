...
function infoFinder(info) {
  return `${info.first_name} ${info.email}`
}
let contactInfo = graduates.map(infoFinder)
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
function uniSorter(hasUni) {
  let universities = hasUni.university;
  return universities.includes("Uni")
}
const uni = graduates.filter(uniSorter);
console.log(uni);
...
*/
const populationTotal = zooAnimals.reduce(function(totalAnimalPop, animal) {
  return totalAnimalPop +=  animal.population
}, 0);
